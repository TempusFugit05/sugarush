...
public partial class Turret : Enemy
{
    ...
    public override void _Ready()
	{
		...
		Weapon = WeaponHolder.GetNode<Weapon>("TurretWeapon");
        Weapon.SetAttachmentMode(Weapon.AttachmentModeEnum.Creature);
    ...
    public override void _PhysicsProcess(double delta)
	{
        if (PlayerNode is not null)
        {
            ...
            if (IsPlayerVisible())
            {
                ...
                if(Weapon.ReadyToShoot() && IsLookingAtTarget(WeaponHolder, PlayerNode))
                {
                    Weapon.Shoot();
                }
            }
